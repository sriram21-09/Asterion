...
def test_compare_cases_success(client, test_db_session):
    ...
    events1 = [
        MovementEvent(case_id=case1.id, sequence_number=1, event_type="location_update", from_cgi="CGI-A", to_cgi="CGI-B", distance_from_prev_m=100.0),
        MovementEvent(case_id=case1.id, sequence_number=2, event_type="location_update", from_cgi="CGI-B", to_cgi="CGI-C", distance_from_prev_m=200.0),
    ]
    
    # Movement Events for Case 2
    events2 = [
        MovementEvent(case_id=case2.id, sequence_number=1, event_type="location_update", from_cgi="CGI-B", to_cgi="CGI-D", distance_from_prev_m=500.0),
    ]
    test_db_session.add_all(events1 + events2)
    
    # Confidence Results
    conf1 = ConfidenceResult(case_id=case1.id, confidence_score=0.8, confidence_level="high", method="gdop")
    conf2 = ConfidenceResult(case_id=case1.id, confidence_score=0.9, confidence_level="high", method="gdop")
    conf3 = ConfidenceResult(case_id=case2.id, confidence_score=0.6, confidence_level="medium", method="gdop")
