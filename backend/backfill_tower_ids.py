...
def main():
    ...
    tower_map = {t.tower_id: (t.latitude, t.longitude) for t in towers if t.latitude is not None and t.longitude is not None}
    ...
    for m in measurements:
        if m.measurement_code.startswith("IMP-"):
            ...
            cdr = db.query(CDRRecord).filter(
                CDRRecord.case_id == m.case_id,
                CDRRecord.timestamp == m.timestamp
            ).first()
            ...
            if cdr:
                ...
                if cgi:
                    ...
                    me = db.query(MovementEvent).filter(
                    ...
                    if m.latitude is not None and m.longitude is not None and cgi in tower_map:
                        ...
                        a = math.sin(dlat/2)**2 + math.cos(math.radians(m.latitude)) * math.cos(math.radians(t_lat)) * math.sin(dlon/2)**2
