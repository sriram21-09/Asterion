...
def test_case_model_creation(db_session):
    ...
    # Retrieve the case
    retrieved_case = db_session.query(Case).filter_by(title="Test Investigation Case").first()
...
def test_scenario_model_creation(db_session):
    ...
    retrieved_scenario = db_session.query(Scenario).filter_by(name="Test Urban Scenario").first()
    assert retrieved_scenario is not None
    assert retrieved_scenario.id is not None
    assert retrieved_scenario.description == "An urban environment scenario configuration."
