...
def generate_report(case_id: int, report_type: str = "full", db: Session = Depends(get_db)):
...
def download_report(case_id: int, db: Session = Depends(get_db)):
    ...
    case_reports = [f for f in os.listdir(report_dir) if f.startswith(f"report_case_{case_id}_") and f.endswith(".pdf")]
    
    if not case_reports:
        raise HTTPException(status_code=404, detail="Report not found for this case. Generate one first.")
    
    # Sort to get the most recent one (using modification time)
    case_reports.sort(key=lambda f: os.path.getmtime(os.path.join(report_dir, f)), reverse=True)
