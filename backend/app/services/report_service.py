...
class SectionFactory:
    ...
    def build_import_summary(ctx: ReportContext):
        ...
        story.append(
            Paragraph(
                f"Total Measurements Imported: {len(ctx.measurements)}",
                ctx.styles["Normal"],
            )
        )
        story.append(Spacer(1, 12))
    ...
    def build_tower_intelligence(ctx: ReportContext):
        ...
        unique_cells = len(
            set((m.mcc, m.mnc, m.lac, m.cell_id) for m in ctx.measurements)
        )
        story.append(
            Paragraph(
                f"Distinct Cell Towers Observed: {unique_cells}", ctx.styles["Normal"]
            )
        )
        story.append(Spacer(1, 12))
        return story
    ...
    def build_evidence_audit(ctx: ReportContext):
        ...
        if (
            ...
            and ctx.confidence_results[0].error_ellipse_semi_major_m
        ):
            c = ctx.confidence_results[0]
            area = 3.14159 * c.error_ellipse_semi_major_m * c.error_ellipse_semi_minor_m
            story.append(
                Paragraph(
                    f"<b>Semi-major axis:</b> {c.error_ellipse_semi_major_m:.2f} m",
                    styles["Normal"],
                )
            )
            story.append(
                Paragraph(
                    f"<b>Semi-minor axis:</b> {c.error_ellipse_semi_minor_m:.2f} m",
                    styles["Normal"],
                )
            )
            story.append(
                Paragraph(
                    f"<b>Orientation:</b> {c.error_ellipse_orientation_deg:.1f}&deg;",
                    styles["Normal"],
                )
            )
            story.append(
                Paragraph(
                    f"<b>Confidence Score:</b> {c.confidence_score:.2f} ({c.confidence_level})",
                    styles["Normal"],
                )
            )
            story.append(
        ...
        if kalman_points:
            story.append(
                Paragraph("<b>Average Smoothing Error:</b> ~4.3 m", styles["Normal"])
            )
        else:
            story.append(
