...
def init_db():
    ...
    try:
        ...
        if isinstance(e, sqlalchemy.exc.DatabaseError) or "file is not a database" in str(e):
            logger.critical("SQLite database is corrupted! Renaming to .corrupt and trying again...")
            try:
                ...
                command.upgrade(alembic_cfg, "head")
                logger.info("Successfully recovered by creating a fresh database. Old database saved as .corrupt.")
